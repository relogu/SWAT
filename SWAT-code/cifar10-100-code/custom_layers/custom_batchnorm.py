...
class custombatchnorm2d(Function):
    ...
    def forward(
        self, input, weight, bias, running_mean, running_var, training, momentum, eps
    ):
        training = float(training) == 1
        momentum = float(momentum)
        eps = float(eps)
        output, save1, save2 = mypkg.myBatchNormForward(
            input, weight, bias, running_mean, running_var, training, momentum, eps
        )

        self.save_for_backward(
            input, weight, running_mean, running_var, save1, save2, torch.tensor(eps)
        )
        return output
    ...
    def backward(self, grad_output):
        ...
        a, b, c = mypkg.myBatchNormBackward(
            input,
            grad_output,
            weight,
            running_mean,
    ...
    def forward(self, input):
        ...
        if self.training or not self.track_running_stats:
            return custombatchnorm2d()(
                ...
                self.running_var,
                torch.tensor(1),
                torch.tensor(exponential_average_factor),
...
if __name__ == "__main__":
    ...
    custom_grad_input = (
        custom_grad_output
    ) = custom_grad_weight = custom_grad_bias = torch.empty(1)
    ...
    for i in range(20):
        compare_bn(model.batchnorm, ref_model.batchnorm)
        x = Variable(
            torch.randn(batch_size, input_channel, input_xdim, input_ydim),
            requires_grad=True,
        )
        x = x.to(device)
        ref_x = x.clone()
