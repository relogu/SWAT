...
if __name__ == "__main__":
    ...
    def compare_linear(l1, l2):
        ...
        if not torch.allclose(l1.weight, l2.weight):
            print("Diff in weight: {} vs {}".format(l1.weight, l2.weight))
            err = True

        if not torch.allclose(l1.bias, l2.bias):
            print("Diff in bias: {} vs {}".format(l1.bias, l2.bias))
            err = True
    ...
    for i in range(10):
        ...
        print(
            "Linear1",
            (model.linear1.weight.grad == ref_model.linear1.weight.grad).sum()
            == model.linear1.weight.numel(),
        )
        print(
            "Linear1",
            (model.linear1.bias.grad == ref_model.linear1.bias.grad).sum()
            == model.linear1.bias.numel(),
        )
        print(
            "Linear2",
            (model.linear2.weight.grad == ref_model.linear2.weight.grad).sum()
            == model.linear2.weight.numel(),
        )
        print(
            "Linear2",
            (model.linear2.bias.grad == ref_model.linear2.bias.grad).sum()
            == model.linear2.bias.numel(),
        )
        ref_optimizer.step()
        optimizer.step()
        print("-----------------")
