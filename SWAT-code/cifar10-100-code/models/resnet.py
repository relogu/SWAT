...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, numchannels, num_classes):
        ...
        self.conv1 = C.CustomConv2d(
            numchannels, 64, kernel_size=3, stride=1, padding=1, bias=True
        )
        layer_number[0] += 1  # increment the layer number
        self.bn1 = B.CustomBatchNorm2d(64)
        self.layer1 = self._make_layer(
            block, 64, num_blocks[0], stride=1, layer_no=layer_number
        )
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], stride=2, layer_no=layer_number
        )
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], stride=2, layer_no=layer_number
        )
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, layer_no=layer_number
        )
...
def test():
    global numC, numL
    net = ResNet18(3, 10)
    net.count_conv_linear_layers(net)
    print(numC, numL)
    pdb.set_trace()
    y: torch.Tensor = net(torch.randn(1, 3, 32, 32))
    print(y.size())
